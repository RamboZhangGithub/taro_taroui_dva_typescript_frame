...
function buildTemplateName(name, pascal = true) {
    ...
    const words = pascal ? utils_1.pascalName(name + '-tmpl') : name + '-tmpl';
    // return words
    const str = [];
...
function getWXMLsource(dirPath, src, type) {
    try {
        const file = fs.readFileSync(path_1.resolve(dirPath, src), 'utf-8');
        return file;
    }
    catch (e) {
...
function parseModule(jsx, dirPath, type) {
    ...
    if (path_1.extname(dirPath)) {
        dirPath = path_1.dirname(dirPath);
    }
    ...
    if (type === 'import') {
        ...
        const { wxml } = wxml_1.parseWXML(path_1.resolve(dirPath, srcValue), wxmlStr, true);
        try {
            if (wxml) {
                block.children = [wxml];
